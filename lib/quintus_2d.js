...
Quintus["2D"] = function(Q) {
 ...
 Q.Sprite.extend("TileLayer",{
  ...
  Q.component('aiBounce', { // he necesitado hacer cambios para que no rebote el enemigo cuando es golpeado por un arma o por el propio jugador
    added: function() {
     this.entity.on("bump.right",this,"goLeft");
     this.entity.on("bump.left",this,"goRight");
	  
	  /*this.on("bump.right",function(collision) {  
				if(!collision.obj.isA("Simon") && collision.obj.p.tipo != "arma"){ // si da al jugador o llega al maximo de golpes que puede dar se elimina
					this.goLeft();
